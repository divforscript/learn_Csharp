// Guided Project - Create an application to manage data about stray or
// abandoned pets.
...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            {
                string currentValue = ourAnimals[i, 2].Remove(0, 5);
                validEntry = int.TryParse(currentValue, out petAge);
                while (!validEntry)
                {
                ...
                // Validate wheter Physical description field is empty -> string at index 3 in the array ourAnimals[i,?]

                validEntry = true;
                currentValue = ourAnimals[i, 4].Remove(0, 22);
                bool onlySpacesString = currentValue.Replace(" ", "").Length == 0 ? true : false;
                ...
                while (!validEntry)
                {
                    ...
                    if (readResult != null)
                    {
                        ...
                        onlySpacesString = animalPhysicalDescription.Replace(" ", "").Length == 0 ? true : false;

                        if (animalPhysicalDescription == null || animalPhysicalDescription.Length == 0 || onlySpacesString)
                        {
                            validEntry = false;
                        }
